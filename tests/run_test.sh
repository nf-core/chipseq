...
data_dir=${data_path}/nfcore-chipseq_test_set
if [ -d $data_dir ]
then
    echo "Found existing test set, using $data_dir"
else
    echo "Downloading test set..."
    curl https://export.uppmax.uu.se/b2013064/test-data/nfcore-chipseq_test_set.tar.bz2 > ${data_path}/nfcore-chipseq_test_set.tar.bz2
    echo "Unpacking test set..."
    tar xvjf ${data_path}/nfcore-chipseq_test_set.tar.bz2 -C ${data_path}
    echo "Done"
fi
