...
### Create IGV session files

<details markdown="1">
    <summary>Output files</summary>

- `igv/<PEAK_TYPE>/`
  - `igv_session.xml`: Session file that can be directly loaded into IGV Desktop.
  - `igv_files.txt`: File containing a listing of the files used to create the IGV session.
- `igv_session.json`: IGV.js browser configuration file published to the **top level** of the results directory.
...
#### IGV.js / Seqera Data Explorer (JSON)

The pipeline also produces an `igv_session.json` file at the root of the results directory. This file follows the [IGV.js `createBrowser()`](https://igv.org/doc/igvjs/) configuration schema and includes:

- **Alignment tracks** (BAM + BAI) for each IP sample and input control
- **Signal tracks** (bigWig) for each sample
- **Peak tracks** (narrowPeak or broadPeak) from MACS3
- **Consensus peak tracks** (BED), when generated

Each sample is assigned a distinct colour, with input controls shown in grey. Track heights are set to sensible defaults (200 px for alignments, 100 px for signal, 50 px for peaks). All URLs are **relative paths** from the output directory root, so they resolve automatically when served from a cloud bucket.

**Seqera Data Explorer integration:** When pipeline results are stored in a cloud bucket accessible via [Seqera Platform](https://seqera.io/), Data Explorer will detect the IGV.js schema and render an embedded genome browser directly in the browser — click `igv_session.json` to load all tracks instantly, no Studios session required.

The `genome` field in the JSON is derived from the `--genome` parameter (e.g. `GRCh38` → `hg38`, `GRCm38` → `mm10`). If your genome is not in IGV.js's built-in list, you may need to supply a custom reference configuration.
